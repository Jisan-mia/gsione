...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const siteLastModified = latestContentDate ? new Date(latestContentDate) : new Date()
  ...
  const blogRoutes: MetadataRoute.Sitemap = blogPosts.map((post) => ({
    ...
    lastModified: new Date(post.updatedAt || post.publishedAt || siteLastModified),
    changeFrequency: 'monthly',
    priority: 0.7,
  }))

  const trainingRoutes: MetadataRoute.Sitemap = trainingPrograms.map((program) => ({
